...
#### `Data`
Determines the data that is sent through the event, can be any [type](./4-types/0-overview.mdx).<br/>
...
Type packs can be defined as a list of [types](./4-types/0-overview.mdx) seperated by a comma within parenthesis.<br/>
For example, a type pack of different [number types](./4-types/2-numbers.mdx) can be written like so:
```ts  copy
