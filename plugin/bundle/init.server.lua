...
[[local Buffer, Size, Instances = SendBuffer, SendCursor, SendInstances]],
        ...
        function Declarations.Event(Declaration)
            ...
            if Value.From ~= Context then
                ...
                Block:Compare(Connection, 'nil', 'Not'):Line(Value.Call == 'SingleSync' and `{Connection}({ListenerArguments})` or `task.spawn({Connection}, {ListenerArguments})`):Branch('Default'):Compare(`#{Queue}`, '256', 'Greater'):Line(`warn("[Blink]: Event queue of \\"{Value.Identifier}\\" exceeded 256, did you forget to implement a listener?")`):End():Line(`table.insert({Queue}, \{{ListenerArguments}} :: \{any})`):End()
